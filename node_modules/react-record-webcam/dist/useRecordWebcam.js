...
const DO_API_URL='https://us-central1-samtv-7b912.cloudfunctions.net/app'
            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    filename = (args === null || args === void 0 ? void 0 : args.downloadFileName) ? args.downloadFileName + ".mp4"
                        ...
                        console.log("step 1")
                        console.log("step 2")
                        const ah =   localStorage.getItem("title");
                        const bod = {
                            title:ah,
                            url:''
                        }
                        const send = async()=>{
                            ...
                            var storageRef=firebase.default.storage().ref('videos/'+filename)
                            await storageRef.put(blob).then((response)=>{
                                console.log('Done',response)
                            }).then(()=>{
                                const getDUrl= async ()=>{
                                var dUrl=await firebase.default.storage().ref('videos/'+filename).getDownloadURL()
                                console.log(dUrl)
                                bod.url=dUrl
                                
                                let body=JSON.stringify(bod)
                                await fetch(`${DO_API_URL}/api/createVideo/`, {
                                    method: 'PUT',
                                    headers: {
